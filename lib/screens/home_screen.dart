...
class _HomeScreenState extends State<HomeScreen>
  ...
  static const Duration _personRecognitionInterval = Duration(seconds: 1);
  ...
  static const Set<String> _humanClassLabels = {'person', 'living'};
  ...
  void initState() {
    ...
    _devanagariTextRecognizer = TextRecognizer(
      script: _scriptByName('devanagari'),
    );
  ...
  Widget _buildScanningState() {
    final statusText =
        _cameraError != null
            ...
            : !_cameraReady
            ? 'Starting camera...'
            : _personRecognitionService.isConfigured
            ? 'Reading signboards and nearby people...'
            : 'Reading signboards...';
      ...
      children: [
        ...
        Positioned(
          ...
          child: Column(
            ...
            children: [
              _buildVoiceOverlay(),
              if (_personRecognitionError != null ||
                  _latestPersonMessage.isNotEmpty) ...[
                const SizedBox(height: 12),
                _buildPersonRecognitionOverlay(),
              ],
                        ...
                        children: [
                          AnimatedBuilder(
                            ...
                            builder: (context, child) {
                              final scale =
                                  isListening ? _micPulseAnimation.value : 1.0;
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  isActive ? const Color(0xFF1A56DB) : const Color(0xFFB3D4FF),
              width: 2,
            ),
            boxShadow: const [
              BoxShadow(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  isActive ? const Color(0xFF2F9E44) : const Color(0xFFB3D4FF),
              width: 2,
            ),
            boxShadow: const [
              BoxShadow(
  ...
  Future<void> _initializeYolo() async {
    ...
    try {
      ...
      setState(() {
        _yoloReady = false;
        _yoloError = _friendlyModelInitError(
          error,
          YOLO_MODEL_ASSET,
          featureName: 'road object detection',
        );
      });
    }
  }
  ...
  Future<void> _initializeIndoorYolo() async {
    ...
    try {
      ...
      setState(() {
        _indoorReady = false;
        _indoorError = _friendlyModelInitError(
          error,
          INDOOR_YOLO_MODEL_ASSET,
          featureName: 'indoor detection',
        );
      });
    }
  }
  ...
  Future<void> _initializeSignboardYolo() async {
    ...
    try {
      ...
      setState(() {
        ...
        _signboardError = _friendlyModelInitError(
          error,
          SIGNBOARD_YOLO_MODEL_ASSET,
          featureName: 'signboard detection',
        );
      });
    }
  }

  String _friendlyModelInitError(
    Object error,
    String assetPath, {
    ...
    if (message.contains('Unable to load asset') ||
      ...
      return 'Missing model asset for $featureName: $assetPath';
    }
    return 'Detector init failed: $message';
  }
  ...
  Future<void> _requestConversationQuestion({
    required bool initialPrompt,
  }) async {
    if (!_cameraReady) {
      if (_soundEnabled) {
  ...
  Widget _buildSmartOverlay() {
    ...
    final hasError =
        (signboardError != null && signboardError.isNotEmpty) ||
  ...
  Widget _buildPersonRecognitionOverlay() {
    final hasError =
        _personRecognitionError != null && _personRecognitionError!.isNotEmpty;
    final title = hasError ? 'Person recognition unavailable' : 'Nearby person';
    final body = hasError ? _personRecognitionError! : _latestPersonMessage;
    final accentColor =
        hasError ? const Color(0xFFFFA3A3) : const Color(0xFFAEE6FF);
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                hasError ? Icons.error_outline : Icons.face_retouching_natural,
                color: accentColor,
                size: 20,
              ),
              const SizedBox(width: 8),
  ...
  Widget _buildDetectionOverlay() {
    ...
    final roadAlerts = _filterAlertDetections(
      _latestDetections,
      _roadAlertLabels,
    );
    final indoorAlerts = _filterAlertDetections(
      _latestIndoorDetections,
      _indoorAlertLabels,
    );
    String roadTitle;
    String roadBody;
  ...
  List<YoloDetection> _filterAlertDetections(
    ...
    final alerts =
        detections.where((d) {
          ...
          final minConfidence =
              isIndoor
                  ? (_indoorMinConfidenceByLabel[normalized] ??
                      _indoorMinConfidence)
                  : (_roadMinConfidenceByLabel[normalized] ??
                      _roadMinConfidence);
  ...
  List<YoloDetection> _applyTemporalStability(
    ...
    final now = DateTime.now();
    history.removeWhere(
      (_, value) => now.difference(value.seenAt) > _stabilityWindow,
    );
  ...
  YoloDetection? _pickMostUrgentAlert(
    ...
    final combined = <YoloDetection>[...roadAlerts, ...indoorAlerts];
  ...
  List<YoloDetection> _mapRoadDetectionsToHazards(
    List<YoloDetection> detections,
  ) {
  ...
  YoloDetection? _detectDarkPatch(img.Image image) {
    ...
    final rectWidth = (maxX - minX + step).toDouble().clamp(
      0.0,
      width.toDouble(),
    );
    final rectHeight = (maxY - minY + step).toDouble().clamp(
      0.0,
      height.toDouble(),
    );
  ...
  String _extractTextFromSignboards(
    ...
    Rect? bestRectFor(Rect rect, double minOverlap) {
      ...
      for (final r in rects) {
        ...
        final overlap =
            (inter.width * inter.height) / (rect.width * rect.height);
  ...
  void _updateSignboardLandmark(Rect rect) {
    ...
    final blended =
        Rect.lerp(current, rect, _signboardLandmarkSmoothing) ?? rect;
  ...
  Future<void> _speakSignboard(String text, YoloDetection detection) async {
    ...
    if (_voiceAssistant.isListening.value || _voiceAssistant.isSpeaking.value)
      return;
  ...
  String _buildAlertMessage(YoloDetection detection) {
    ...
    final normalized = _normalizeLabelForAlert(detection.label);
    final spokenLabel =
        _alertSpokenLabels[normalized] ?? _humanizeLabel(detection.label);
    final direction = _directionForRect(detection.rect);
  ...
  Future<void> _maybeSpeakAlerts() async {
    ...
    if (_voiceAssistant.isListening.value || _voiceAssistant.isSpeaking.value)
      return;
    final now = DateTime.now();
    if (now.difference(_lastSignboardSpokenAt) < _signboardPriorityWindow)
      return;
    ...
    if (key == _lastAlertKey &&
        now.difference(_lastAlertSpokenAt) < const Duration(seconds: 8)) {
      return;
    }
  ...
  Future<void> _processCameraImage(CameraImage image) async {
    ...
    _frameIndex++;
    try {
      final bool shouldRunPersonRecognition =
          _personRecognitionService.isConfigured &&
      ...
      if (shouldRunPersonRecognition) {
        ...
        if (personImage != null) {
          ...
          unawaited(_runPersonRecognition(personImage));
        }
      }
      ...
      if (_pendingConversationQuestion != null && !_conversationInFlight) {
        ...
        final conversationText =
            mergedText.trim().isNotEmpty ? mergedText : _lastIntentOcrText;
      ...
      if (_intentModeEnabled &&
        ...
        debugPrint(
          '[INTENT] Frame check: mode=enabled query="$query" ocr=${mergedText.length}chars',
        );
        ...
        if (rgbImage != null) {
          if (shouldRunRoad) {
            try {
              ...
              final scaledDetections = _scaleDetectionsToFrame(
                detections,
                rgbImage,
              );
              ...
              final darkPatch =
                  hasRoadContext ? _detectDarkPatch(rgbImage) : null;
              final scaledDarkPatch =
                  darkPatch == null
                      ? null
          ...
          if (shouldRunSignboard) {
            try {
              ...
              if (mounted) {
                setState(() {
                  _signboardError =
                      'Signboard detector error: ${error.toString()}';
                });
              }
            } finally {
  ...
  Uint8List _convertToNv21(CameraImage image) {
    ...
    for (int row = 0; row < height ~/ 2; row++) {
      ...
      for (int col = 0; col < width ~/ 2; col++) {
        ...
        if (uvOffset >= uPlane.bytes.length ||
            uvOffset >= vPlane.bytes.length) {
          continue;
        }
    ...
    if (Platform.isIOS) {
      ...
      if (description.lensDirection == CameraLensDirection.front) {
        ...
        rotationCompensation =
            (description.sensorOrientation - rotation + 360) % 360;
      }
    }
  ...
  void _handleOcrResult(String text, CameraImage image) {
    ...
    final now = DateTime.now();
    if (normalized == _lastEmittedText &&
        now.difference(_lastEmitTime) < _emitCooldown) {
      return;
    }
    ...
    if (_lastEmittedText.isNotEmpty) {
      ...
      if (similarity >= 0.9 &&
          now.difference(_lastEmitTime) < const Duration(seconds: 4)) {
        return;
      }
    }
  ...
  Future<void> _maybeSendToGemini(String text, CameraImage image) async {
    ...
    try {
      ...
      if (result.action != null) {
        debugPrint(
          'Gemini action: ${result.action!.type} -> ${result.action!.value}',
        );
      }
  ...
  Future<void> _checkIntentMatch(
    ...
    final bestOcr =
        _lastIntentOcrText.trim().isNotEmpty
            ? _lastIntentOcrText.trim()
            : fullOcr;
    ...
    try {
      ...
      await _announceIntentMatch(
        _activeIntentQuery ?? (result.category ?? intent),
        focusRect,
      );
    } catch (error) {
  ...
  Future<void> _announceIntentMatch(String intentName, Rect? focusRect) async {
    ...
    final now = DateTime.now();
    final direction =
        focusRect != null ? _directionForRect(focusRect) : 'ahead';
    ...
    if (key == _lastIntentKey &&
        now.difference(_lastIntentSpokenTime) < _intentCooldown) {
      return;
    }
  ...
  Future<void> _runPersonRecognition(Uint8List jpegBytes) async {
    try {
      ...
      final overlayMessage = switch (result.status) {
        'known' || 'unknown' => result.message,
        'too_far' => 'Person detected, but too far for recognition.',
        'face_not_found' => 'Person detected, but face not clearly visible.',
        _ => '',
      };
      if (mounted) {
        setState(() {
          _personRecognitionError = null;
          _latestPersonMessage = overlayMessage;
        });
      }
      if (!_soundEnabled ||
      ...
      await _voiceAssistant.speak(result.message);
    } catch (error) {
      if (!mounted) return;
      setState(() {
        _personRecognitionError =
            'Person recognition error: ${error.toString()}';
        _latestPersonMessage = '';
      });
    } finally {
  ...
  String _formatPrice(_MenuItem item) {
    final hasDecimals =
        (item.price - item.price.truncateToDouble()).abs() > 0.001;
    final value =
        hasDecimals
  ...
  List<String> _intentKeywordsForQuery(String intent) {
    ...
    for (final entry in _intentKeywordHints.entries) {
      ...
      final hasMatch = intentTokens.any(
        (token) =>
