...
class GeminiService {
  GeminiService({String? apiKey, String? model})
    : _apiKey = _resolveConfigValue(
        ...
        dotenvValue: dotenv.isInitialized ? dotenv.env['GEMINI_API_KEY'] : null,
      ),
      _model = _resolveConfigValue(
        override: model,
        dartDefine: const String.fromEnvironment('GEMINI_MODEL'),
        dotenvValue: dotenv.isInitialized ? dotenv.env['GEMINI_MODEL'] : null,
  ...
  static String _resolveConfigValue({
    ...
    for (final candidate in [override, dartDefine, dotenvValue]) {
...
''';
    ...
    final body = jsonEncode({
      ...
      'generation_config': {'temperature': 0.2, 'max_output_tokens': 140},
    });
